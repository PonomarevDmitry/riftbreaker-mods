...
function mech:init()
	...
	-- MOD START
	-- 
	self.allowedweaponsubtype = "NA"
	self.srq = self:CreateStateMachine()
	self.srq:AddState( "checkweapons", { execute = "OnWeaponCheck" })
...
function mech:OnWeaponCheck( state )
	local L_hand = ItemService:GetEquippedItem( self.entity, "LEFT_HAND" )
	local R_hand = ItemService:GetEquippedItem( self.entity, "RIGHT_HAND" )

	local L_hand_name = ItemService:GetItemName(L_hand)
	local R_hand_name = ItemService:GetItemName(R_hand)

	local L_hand_sub = ItemService:GetItemSubType( L_hand )
	local R_hand_sub = ItemService:GetItemSubType( R_hand )

	ConsoleService:Write("L-Hand Name: " .. tostring(L_hand_name))
	ConsoleService:Write("L-Hand Sub: " .. tostring(L_hand_sub))

	ConsoleService:Write("R-Hand Name: " .. tostring(R_hand_name))
	ConsoleService:Write("R-Hand Sub: " .. tostring(R_hand_sub))


	if L_hand ~= nil then
		if L_hand_sub ~= self.allowedweaponsubtype then
			if L_hand_sub ~= "melee" then
				EntityService:RemoveEntity( L_hand )
			end
		end
	end
	
	if R_hand ~= nil then
		if R_hand_sub ~= self.allowedweaponsubtype then
			if R_hand_sub ~= "melee" then
				EntityService:RemoveEntity( R_hand )
			end
		end
	end

	LogService:DebugText( 1120, 20, "Allowed weapon type: " .. tostring(self.allowedweaponsubtype), "debug_white_size_38" )
end



function mech:RollAllowedWeaponSubTypes()
	local weapon_subtype_array = { "explosive", "low_caliber", "energy", "liquid", "high_caliber" }

	local idx = RandInt(1, #weapon_subtype_array)
	
	self.allowedweaponsubtype = weapon_subtype_array[idx]
	
	ConsoleService:Write(tostring(self.allowedweaponsubtype))
...
function mech:OnShockwaveExit( state )
	...
	self:RollAllowedWeaponSubTypes()
	QueueEvent( "LuaGlobalEvent", event_sink, "InitialSpawnEnded", {} )
	self.srq:ChangeState("checkweapons") --- MOD
end
