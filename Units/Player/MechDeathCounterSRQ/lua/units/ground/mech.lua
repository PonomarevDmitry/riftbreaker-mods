...
function mech:init()
	...
	self.srq:AddState( "deathcounter", { execute = "OnDeathCount" })
	self.srq:ChangeState("deathcounter")
...
function mech:OnDeathCount()
	LogService:DebugText( 1320, 20, "Deaths: " .. tostring(deathcount), "debug_white_size_38" ) -- Change the first value, 1320, to change the left <-> right position, bigger number = further right ;;; Second number, 20, to change the up /\-\/ down position, bigger number = furhter down
end
