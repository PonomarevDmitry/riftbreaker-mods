...
require("lua/utils/reflection.lua") -- MOD
require("lua/utils/numeric_utils.lua") -- MOD
...
function mech:init()
	...
	self.srq:AddState( "speedcounter", { execute = "OnSpeedCount" })
	self.srq:ChangeState("speedcounter") -- MOD
...
function mech:OnSpeedCount()
	local speedcount = reflection_helper( EntityService:GetComponent(self.entity, "MechComponent"))  -- Get the MechComponent which holds the values for the speed (=velocity)
	if speedcount ~= nil then
		local speed = Length( speedcount.velocity ) -- Do some math magic and turn the Vector3 velocity into a general speed
		LogService:DebugText( 1320, 20, "Speed: " .. tostring(speed), "debug_white_size_38" ) -- Change the first value, 1320, to change the left <-> right position, bigger number = further right ;;; Second number, 20, to change the up /\-\/ down position, bigger number = further down
	end
end
