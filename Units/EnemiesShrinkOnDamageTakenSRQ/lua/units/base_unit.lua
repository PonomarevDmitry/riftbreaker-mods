...
function base_unit:Rescale()
	local defEntScale = EntityService:GetScale(self.entity)
	if (defEntScale.x - 0.2) > 0 and (defEntScale.y - 0.2) > 0 and(defEntScale.z - 0.2) > 0 then
		EntityService:SetScale(self.entity, defEntScale.x - 0.2, defEntScale.y - 0.2, defEntScale.z - 0.2)
	else
		EntityService:ChangeType( self.entity, "quelver" ) -- You can run over tiny enemies and kill them!
	end
end
