...
function mech:init()
	...
	self.srq:AddState( "walk", { execute = "OnWalk" })
	self.srq:ChangeState("walk") -- MOD
...
function mech:OnWalk()
	local srqtimeofday = EnvironmentService:GetTimeOfDay()
	if srqtimeofday ~= "day" then
		local component = reflection_helper( EntityService:GetComponent(self.entity, "MechComponent"))   -- Get the MechComponent which holds the values for the max speed (=max_speed)
		if component ~= nil then
			component.max_speed = 0
		end
	else
		local component = reflection_helper( EntityService:GetComponent(self.entity, "MechComponent"))   -- Get the MechComponent which holds the values for the max speed (=max_speed)
		if component ~= nil then
			component.max_speed = 15
		end
	end
end
