...
local PlayStats = require("lua/services/playstats_utils.lua") -- MOD
...
function mech:init()
	...
	self.srq:AddState( "neutralkillcheck", { execute = "CheckForKill" })
	self.srq:ChangeState("neutralkillcheck")
...
function mech:CheckForKill()
	ConsoleService:Write("Check")
	local GetSumNeutralUnitsKilled = PlayStats:GetSumNeutralUnitsKilled( )
	if (GetSumNeutralUnitsKilled > 0 ) then
